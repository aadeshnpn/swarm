...
class Sampling:
    ...
    def gen_plot(self):
        ...
        fnames = [
            'foodonly.npy', 'exploreonly.npy', 'expfood.npy', 'cfandfood.npy', 'expcf.npy', 'all.npy']
        data = []
        for fname in fnames:
            val = np.load(self.directory+ '/' + str(fname))
            vallast = val[:,4999]
            data.append(vallast)
    ...
    def plot(self, datas, rate):
        ...
        bp1 = ax1.boxplot(
            data.T, 0, 'gD', showmeans=True, meanline=True, patch_artist=True)
        ...
        func = [
            ...
            '4. Prospective + Task-Specific', '5. Exploration + Prospective ', '6. All']
        ax1.legend(zip(bp1['boxes']), func, fontsize="small", loc="upper left")

        plt.tight_layout()
        fig.savefig(self.directory + '/' + 'fitness.pdf')    # pylint: disable=E1101
        fig.savefig(self.directory + '/' + 'fitness.png')    # pylint: disable=E1101
        plt.close(fig)

def main():
    """Parse args and call graph module."""
    # fdir = "/home/aadeshnpn/Documents/BYU/hcmi/hri/ijcai/numpy_diveristy"
