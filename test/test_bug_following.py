...
# Class to tets Passable attribute for agents
class SwarmAgentTrapGoTo(Agent):
    """ An minimalistic behavior tree for swarm agent implementing goto
    behavior
    """
    def __init__(self, name, model):
...
class GoToSwarmEnvironmentTrapModel(Model):
    """ A environemnt to model swarms """
    def __init__(self, N, width, height, grid=10, seed=None):
        if seed is None:
            super(GoToSwarmEnvironmentTrapModel, self).__init__(seed=None)
        else:
            super(GoToSwarmEnvironmentTrapModel, self).__init__(seed)
...
class TestGoToSwarmTrapSmallGrid(TestCase):

    def setUp(self):
        self.environment = GoToSwarmEnvironmentTrapModel(1, 100, 100, 10, 123)
