...
def simulate_res1(env, iteration):
    ...
    sim = SimModel(
        ...
        expname='MSFCommRes1', agent='SimAgentResComm1')
...
def simulate_res2(env, iteration):
    ...
    sim = SimModel(
        100, 100, 100, 10, iter=iteration, xmlstrings=phenotypes, pname=env[1],
        expname='MSFCommRes2Simulate', agent='SimAgentResComm2')
    sim.build_environment_from_json()
...
def evolve(iteration):
    ...
    env = EvolModel(
        100, 100, 100, 10, iter=iteration, expname='MSFCommEvolve',
        agent='EvolAgent', parm='swarm_comm.txt')
