...
class CTModel(Model):
    """A environemnt to model CT environment."""
    ...
    def build_environment_from_json(self):
        ...
        try:
            ...
            for i in range(self.num_agents * 1):
                f = Food(
                    i, location=self.site.location, radius=food_radius)
                f.agent_name = None
...
class EvolveModel(CTModel):
    """A environemnt to model swarms."""

    def __init__(
            self, N, width, height, grid=10, iter=100000,
            seed=None, name="EvoCTForge", viewer=False):
        """Initialize the attributes."""
