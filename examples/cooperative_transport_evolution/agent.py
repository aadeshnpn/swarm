...
from swarms.behaviors.scbehaviors import (
    MoveTowards, CompositeDropPartial,
    CompositeMultipleCarry, Explore
)


class CTAgent(Agent):
    """An minimalistic cooperative transport swarm agent."""
    ...
    def detect_food_carrying(self):
        ...
        if len(self.partial_attached_objects) > 0:
            print('Food carying', self.name, self.partial_attached_objects)
    ...
    def carrying_fitness(self):
        ...
        the agents.
        """
        return sum([obj.weight for obj in self.partial_attached_objects])
...
class LearningAgent(CTAgent):
    ...
    def init_evolution_algo(self):
        ...
        parameter_list = ['--parameters', '../..,swarm_mcarry.txt']
...
class ExecutingAgent(CTAgent):
    """A cooperative transport swarm agent.
