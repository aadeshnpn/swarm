...
def simulate_res1(env, iteration):
    ...
    sim = SimModelRes1(
        150, 100, 100, 10, iter=iteration, xmlstrings=phenotypes, pname=env[1])
...
def main(iter):
    ...
    # simulate(None, iter)
    # Read from the json
    pname = '/home/aadeshnpn/Documents/BYU/HCMI/research/resilience/forg'
    ...
    #if len(phenotypes) > 1:
    steps = [5000 for i in range(20)]
    # env = (env.phenotypes, env.pname)
    #aname = pname + '/' + str(N)
    env = (phenotypes, pname)
    Parallel(n_jobs=16)(delayed(simulate)(env, i) for i in steps)
    Parallel(n_jobs=16)(delayed(simulate_res1)(env, i) for i in steps)
    Parallel(n_jobs=16)(delayed(simulate_res2)(env, i) for i in steps)
    #for i in steps:
    #    simulate(env, i)
