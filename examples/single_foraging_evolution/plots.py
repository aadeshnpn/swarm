...
def boxplot_exp_0():
    agents = [50, 100, 200, 300, 400, 500]
    dataf = [read_data_exp_3(100, 100, 0, 0, exp_no=0, site=30, no_trap=0, no_obs=0, agent=a, grid=10)[:,-1] for a in agents]
    ...
    ax1.legend(zip(bp1['boxes']), ['Foraging'], fontsize="small", loc="upper right", title='Performance Metric')
    ...
    maindir = '/tmp/oldswarm/data/experiments/'
    fname = 'oldagentscompboxplot'
...
def experiment_1(agent='ExecutingAgent'):
    # plt.style.use('fivethirtyeight')
    sites = [20, 25, 30, 40]
    datasf = [read_data_exp_3(100, 100, 0, 0, exp_no=1, site=s, no_trap=0, no_obs=0, grid=10) for s in sites]
    ...
    fig.savefig(
        nadir + 'oldagentallsitecompdistplot' + '.png')
    # fig.savefig(
    #     maindir + '/' + fname + '.png')
    # pylint: disable = E1101
...
def read_data_exp_3(width=100, height=100, trap=0, obs=0, exp_no=3, site=30, no_trap=0, no_obs=0, agent=100, no_site=1, grid=10):
    maindir = '/tmp/oldswarm/data/experiments/'
    if grid is None:
...
def boxplot_exp_3():
    ...
    data = [read_data_exp_3(s, s, 0, 0, exp_no=3, site=30, no_trap=0, no_obs=0, grid=10)[:,-1] for s in size]
    fig = plt.figure(figsize=(8,6), dpi=200)
    ax1 = fig.add_subplot(1, 1, 1)
    colordict = {
        0: 'forestgreen',
...
def boxplot_exp_2():
    size = [ 1, 2, 3, 4, 5]
    dataf = [read_data_exp_3(100, 100, 0, 0, exp_no=2, site=30, no_trap=0, no_obs=0, grid=10, no_site=s)[:,-1] for s in size]
    ...
    # plt.xlim(0, len(mean))
    ax1.legend(zip(bp1['boxes']), ['Foraging'], fontsize="small", loc="upper left", title='Performance Metric')
