from model import SimCoevoModel

# from swarms.utils.jsonhandler import JsonData
from swarms.utils.graph import GraphACC
from joblib import Parallel, delayed    # noqa : F401
from swarms.utils.results import SimulationResultsWTime
...
def simulate_forg(
        ...
        notrap=1, noobs=1, nosite=1, grid=10):
    ...
    sim = SimCoevoModel(
        ...
        nosite=nosite, brepotires=env[2])
...
def main(args):
    ...
    if args.all:
        ...
        if args.exp_no == 0:
            ...
            for nt in range(1, 6):
                pprint(n, agent, site, trap, obs, width, height, no_trap, nt, exp_no, grid, no_site)
                if not args.dry_run:
                    exp(n, agent, site, trap, obs, width, height, no_trap, nt, exp_no, grid, no_site)
        elif args.exp_no == 5:
