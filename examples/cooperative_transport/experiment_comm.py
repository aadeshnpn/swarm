...
def simulate_res1(env, iteration):
    ...
    sim = SimModel(
        ...
        expname='COTCommRes1', agent='SimAgentResComm1')
...
def simulate_res2(env, iteration):
    ...
    sim = SimModel(
        100, 100, 100, 10, iter=iteration, xmlstrings=phenotypes, pname=env[1],
        expname='COTCommRes2', agent='SimAgentResComm2')
    sim.build_environment_from_json()
...
def simulate(env, iteration):
    ...
    sim = SimModel(
        100, 100, 100, 10, iter=iteration, xmlstrings=phenotypes, pname=env[1],
        expname='COTCommSimulate', agent='SimAgent')
    sim.build_environment_from_json()
...
def evolve(iteration):
    ...
    env = EvolModel(
        100, 100, 100, 10, iter=iteration, expname='COTCommEvolve',
        agent='EvolAgent', parm='swarm_mcarry_comm.txt')
